...
export const ScheduleSection: React.FC<SpeakersSectionProps> = ({
  speakers,
  schedule,
}) => {
      ...
      {speakers.length && (
        <Container>
          ...
          {schedule.map((schedule, index) => {
            const commonSpeeches = schedule.speeches.filter(
              (speeches) => speeches.path !== SpeechesPath.COMUNIDADE,
            );
            const speedSpeeches = schedule.speeches.filter(
              (speeches) => speeches.path === SpeechesPath.COMUNIDADE,
            );
